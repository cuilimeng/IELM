function result = test_all_method(trX, teX, split, trK, teK, para)
...
%         predict_response =  teK(:, model.support_v) * model.alp + model.b;
    ...
    case 'alter-pSVM'
        ...
        while(para.C ~= C)
            ...
            if ~isfield(model,'support_v') || ~isfield(model,'obj')
                model = model_pre;
            else 
                model_pre = model;
            end
    ...
    case 'alter-pSVM-pin'
        ...
        while(para.C ~= C)
            para.C = min(para.C*1.5,C);
            model = alternating_svm_pin_dual_with_slack(trX, trK, split.train_bag_idx, split.train_bag_prop, para);
%             if ~isfield(model,'support_v') || ~isfield(model,'obj')
%                 model = model_pre;
%             else 
%                 model_pre = model;
%             end
        ...
        predict_response =  teK * (model.alp) + model.b;
end
...
predicted_Y = result.predicted_train_label; truth_Y = split.train_label;
result.ins_level_acc = (sum(predicted_Y+1 & truth_Y+1)/length(truth_Y)*length(truth_Y(truth_Y==1))+sum(-predicted_Y+1 & -truth_Y+1)/length(truth_Y)*length(truth_Y(truth_Y==-1)))/length(truth_Y);
